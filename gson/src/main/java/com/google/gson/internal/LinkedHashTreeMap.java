...
public final class LinkedHashTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
  ...
  Node<K, V> find(K key, boolean create) {
    ...
    Node<K, V>[] tableBis = this.table;
    int hash = secondaryHash(key.hashCode());
    int index = hash & (tableBis.length - 1);
    Node<K, V> nearest = tableBis[index];
    int comparison = 0;
  ...
  public void refactor(int height,Node<K, V> side,Node<K, V> adjacent) {
	  if (side != null) {
	        height = side.height;
	        adjacent.left = side;
	        side.parent = adjacent;
	        side.left = null;
	  }
  }
  ...
  void removeInternal(Node<K, V> node, boolean unlink) {
    ...
    if (left != null && right != null) {
      ...
      removeInternal(adjacent, false); // takes care of rebellions and size--

      int leftHeight = 0;
      left = node.left;
      int rightHeight = 0;
      right = node.right;
      refactor(leftHeight,left,adjacent);
      refactor(rightHeight,right,adjacent);
  ...
  private void rebalance(Node<K, V> unbalanced, boolean insert) {
    for (Node<K, V> node = unbalanced; node != null; node = node.parent) {
      ...
      if (delta == -2) {
    	rightRebalance(right,insert,node);
    	if (insert)
    		 break;

      } else if (delta == 2) {
        leftRebalance(left,insert,node);
        if (insert) {
  ...
  @Override public Set<K> keySet() {
    KeySet result = aKeySet;
    return result != null ? result : (aKeySet = new KeySet());
  }
  ...
  static final class Node<K, V> implements Entry<K, V> {
    ...
    public Node<K,V> give_the_last_or_first_node_of_subtree( Node<K, V> nodeElement){
      Node<K, V> node = this;
      Node<K, V> child = nodeElement;
      while (child != null) {
        node = child;
        child = nodeElement;
      }
    ...
    public Node<K, V> first() {
        return give_the_last_or_first_node_of_subtree(this.left);
    }
    ...
    public Node<K, V> last() {
      return give_the_last_or_first_node_of_subtree(this.right);
  ...
  final static class AvlBuilder<K, V> {
    ...
    void add(Node<K, V> node) {
      ...
      for (int scale = 4; (size & scale - 1) == scale - 1; scale *= 2) {
        if (leavesSkipped == 0) {
          ...
          make_center_the_top_of_the_stack(right, center);
    ...
    void make_center_the_top_of_the_stack(Node<K, V> right, Node<K, V> center){
        // Pop right and center, then make center the top of the stack.
       stack = center;
       center.right = right;
